...
/**
 * task-40's test case 6. The Escape rule is stated in two prose files — the
 * `CLAUDE.md` bullet and its `invariants.md` entry — and the one thing that can
 * silently rot in both is the COUNT: the code has three callers, the docs said
 * four for as long as it took someone to notice.
 *
 * Read as text, the way `test/claude-rules.test.ts` reads the rule files: these
 * are prose, there is nothing to import, and the point is that the sentences a
 * reader arrives at say what the stack actually does.
 *
 * What is asserted is deliberately narrow — the count, the three names, and
 * that `RunDrawer`'s departure is EXPLAINED rather than merely erased — because
 * a test that pinned whole paragraphs would fail on every rewording and teach
...
/** The CLAUDE.md bullet for the same rule. Bullets are separated by `\n- `. */
function claudeBullet(): string {
  const at = CLAUDE.indexOf('- **Escape has one owner');
  expect(at).toBeGreaterThan(-1);
  const next = CLAUDE.indexOf('\n- ', at + 1);
  return CLAUDE.slice(at, next === -1 ? undefined : next);
}
...
describe('the Escape rule reads three dialogs, not four', () => {
  ...
  it('the CLAUDE.md bullet says the same three and points at the same reason', () => {
    const bullet = claudeBullet();
