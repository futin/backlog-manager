...
**The figure strip, directly under the band — the statistics lead.** One sheet the ground divides: five figures on a 2 px `--board` gap grid, wrapping five → three → two under 1100 and under 700 with the seam drawn in both directions, so the gap reads as one grid at every width rather than as five cards that happen to touch. Each cell is a label at 13 `--ink2`, a value at 30/700, and a 12 px line **only where today's tile already has one** — an invented substat is a figure nobody can check. The five read what `aggregateRuns` returns (`client/src/lib/run-stats.ts:653`) and nothing else, which is what rules out the two readings the mock had drawn: there is no prior-period delta to compare against, and `parked` is not a count the aggregate keeps.
...
- **rework / completed** — the one-decimal ratio `RunAggregates.fixLoopsPerMerged` (`run-stats.ts:588`), with today's long sentence kept as the cell's `title`. The mock's `6 / 41` fraction needs two numbers the aggregate does not return.
- **verify pass** — a rate over verification *runs*, not items; the line says so, because the unit is the whole claim.
...
**Three figures.** **Sweeper** — the phase as a 24/700 word rather than a number, since there is no quantity here, with a 6 px sweep meter under it in `--fill-progress` carrying `.hatch` (§8.2) and the line `next sweep in 4m 12s · every 10m`; the meter draws only while armed. **Watching** — the count of running runs over today's own substat line, `⚠ 1 crashed · 2 fresh · 1 not yet watched`, whose last part draws only when the payload is ahead of the sweeper and all of which gives way to `nothing is watched while off` when the switch is off (`WatchdogMonitor.tsx:193-208`). The spec writes that line as `1 crashed · 1 heartbeating`; `heartbeating` is a word this app does not use, the mockup audit had already replaced it with today's `fresh` and `not yet watched`, and this section follows the code. **Policy** — `check every` / `leave alone for` / `give up after` as three label-over-value pairs, 12 over 14/500, with the enabled switch's state as the 12 px line. Both the sweeper line's cadence and all three policy values are the saved config's, not constants; their bounds are `WATCHDOG_LIMITS` (`shared/types.ts:1029`), and the Settings ladders and the server's clamp already read that one triple.
...
The meter's two labels are the one figure in the spec's §4.2 this section does not repeat. Both are formatted from the constant at render time (`WatchdogMonitor.tsx:316-318`), and the component's own comment gives the reason — a meter labelled with a typed literal keeps naming the old window the day `RUN_STALE_MS` moves. The spec's example labels name a span that is not this constant's value; the rule is `stale at <the constant>` / `past the <the constant> stale line`, and no numeral belongs in this document or in that JSX.
