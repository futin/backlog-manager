...
| `--on-fill` | ink ON either of those two fills | `var(--ink)` | `var(--on-accent)` |

`--on-fill` is `task-37`'s, added when the card's live strip first set type on one of those fills: `--ink` is right on daylight (`#131313` on `#F5A15C`, 7.3:1) and wrong on the four dark themes, where it is near-white against a bright amber — 1.4:1 on midnight, the default, at 11 px. `--on-accent` fails the other way, because on daylight the accents are dark and so that token is white. So the third token is the dark end of whichever palette declares it, and §8.3's "11/500 `--ink` text" is read through it.
...
The four column dots read the design's category ramp left to right — refactors `--amber`, ideas `--mustard`, bugs `--red`, tasks `--green` — replacing today's magenta/mustard/red/cyan ticks. The refactors tick is `--magenta`'s last *column* job, but not its last job: the capture dispatch control still reads it. That was four of the token's five uses in the stylesheet — `.dispatch-tab.capture` and `.dispatch-chip.capture`, base and hover — against the tick's one; `task-37` collapsed the control's two shapes into one `Chip` whose action tone paints the WORD, so the four became `.dispatch-word.capture` alone and the tick survived on Archive's own column header until `task-39` put that surface on `BoardColumn` too, which left `.dispatch-word.capture` as the token's one remaining reader. The dispatch chip keeps its three action tones exactly as coded, unchanged by this redesign: groom `--mustard`, execute `--cyan`, capture `--magenta`. Ideas now sharing mustard with the groom chip costs nothing — a column dot and a card's own control are never read as the same object.
...
- Marker row, drawn whenever a marker applies or dispatch is available (below): `Marker` renders `groomed` in `--green`, `chore`/`debt` in `--ink3`, `done` in `--ink2`, `stale` in `--mustard`, and on a tracker card the `orchestrator:queued` label as `queued` in `--ink2` while a live run holds the project or a dimmed `queued · stale` in `--ink3` (titled with the way out) when none does, all 11/500 — the same words, the same tokens, §7's size.
...
**Amended 2026-09-17 — the wide board.** D was drawn at the 1280 px measure, where the strip can only lead the page. With `contentWidth: full` (§8.6) a 1920 px window gives the board half as much width again, and three prototypes were drawn over the live page at that size and picked between: **A**, the strip standing as a 320 px right rail with the split unchanged beside it — **picked**; **B**, a 480 px panel with the five figures in a 2-column grid and the selected run's own machine time moved up beside the range's so the two bars compare; **C**, B plus the run's facts strip and count chips moved into that panel, leaving the detail sheet its head, attention and items. B and C were rejected together: both move readings out of the run's own sheet to make a panel look full, and C separates the status word from the controls that act on it. The rail is §8.4.1's "Wide" paragraph below; the pick also moved two blocks inside the detail body (see the body list).
...
**Wide** (2026-09-17): once the BOARD is **1400 px** or wider, the page becomes three columns — the band spanning the top, the split holding the first column exactly as above, and the figure strip leaving the top of the page to stand as a **320 px rail** on the right, one figure wide, its five cells over the range's machine-time bars, top-aligned and scrolling itself when the six exceed the board. Nothing else moves: the list stays 420 px, the rows and the detail body are unchanged, and under 1400 px the page is byte-identical to the paragraph above. The bars' label and value columns narrow to 72/56 in the rail so the track keeps about 140 px, below which a seven-row chart stops being comparable — which is the only thing the chart is for.

The threshold is 420 + ~600 + 320 + two `--group-gap`, where ~600 px is the width below which the detail's seven-node stage track stops being legible. The 1280 px `.wrap.wide` cap sits under it deliberately, so the rail is reachable only with content width set to full, and there only on a window around 1728 px or wider at 100 % text.

It is a **container query on a measuring wrapper the History page renders around itself** (`.runs-frame`), and both halves of that are load-bearing. A media query is the wrong instrument: the board's width is the window minus the 280 px rail, minus `--body-pad` twice, minus whatever the measure cap is doing, all divided by `.shell`'s `zoom` — at 120 % text a 1920 px window leaves about 1112 CSS px of board, which is not room for three columns and which `@media (min-width: 1920px)` would fill with them anyway. And the container is that wrapper rather than `.wrap` or `.main` because `container-type: inline-size` applies layout containment, which makes the element the containing block for every `position: fixed` descendant: the item modal and the form sheets (§8.7) are fixed and render inside `.wrap` with no portal, so a container up there would pin them to the section instead of the viewport. Nothing under Runs is fixed today; whatever becomes so must portal to `body`. Watchdog renders no wrapper — it has no strip to stand as a rail. The strip and the band are placed through the page's own classes (`.runs-stats`, `.runs-band`) passed to the primitives as `className`, the contract `Sheet` already had (§12.1), never by selecting `.ui-figure-strip` from the page.
...
**The Watching sheet.** One row per run in the runs payload whose status is `running`, annotated from `watching` exactly as today — skew rendered, not hidden. Each row carries a `Dot`, the project at 14/500, the run id at 12 `--ink3`, `· not yet watched` when the payload is a tick ahead of the sweeper (`client/src/components/runs/WatchdogMonitor.tsx:271`), the verdict as glyph plus word (`● ok` / `⚠ crashed`), `last reported <id> · <stage>` or `between items`, and the heartbeat meter — the age against `RUN_STALE_MS` (`shared/types.ts:767`), amber once past it — under its two labels. A crashed row adds the attempts dots, the `watchdogClause` sentence, `→ session <id>`, and `leave alone Nm more` while the grace clock runs. A `Resume now` chip is drawn **only** when `watchdogStoodDown` allows — never while the sweeper will still spawn (attempts left AND, since bug-35, refusals under the same ceiling), whatever the grace clock says, because that is the one function both this chip and §8.4.1's detail head read. A watched id with no run in the payload renders as a placeholder line rather than as nothing: a set the console cannot explain is the failure this page exists to show.
...
The band carries the project select and search and nothing else — no status filter and no sort control, deliberately: Archive's contents are defined by staleness and rejection rather than by status, so a status select here would either do nothing or contradict the surface it sits on, and the month grouping is already the ordering a sort control would otherwise offer (`ArchiveView.tsx:174-178`). No live strip ever paints here either: whatever put a card in Archive already took it off the board a run could be holding. The one action an Archive card can still derive is `capture` (`shared/agent.ts:102`): reviving a rejection by filing a new item that cites it, the only `AgentAction` an out-of-scope item derives at all (`deriveAction`), and the one dispatch chip standing among cards that are otherwise finished.

### 8.6 Settings

Settings restates the dashboard's own §8.2 rather than deriving a second version of the same card language (`../claude-agents-dashboard/.claude/DESIGN.md:225-246`): a band header — the 19/500 title over a 13 px line, the same band every other page on this board opens with (§8.3) — sits above borderless `--strip` cards at 16 px radius and 24 px padding, each a title plus a one-line subtitle. That pair is what `task-39` split the old one-line group heading into: `SettingsGroup` takes `title` and `scope` separately, because the scope half answers a different question from the name half — whether changing this setting affects anybody but the person changing it — and the five scopes are deliberately three different answers. Four of the five cards keep the grouping `SettingsView.tsx` already ships — `Board · this device`, `Display · this device`, `Orchestrator · this device`, `Claude Agents · this machine` — and the fifth is `WatchdogGroup.tsx`'s own card, `Orchestrator watchdog · this server`: a distinct card from `Orchestrator · this device` beside it, not a second rendering of it, rendered from its own file rather than `SettingsView.tsx`'s. The redesign restyles the cards, not what they group.

Rows inside stay boxless: 16 px of vertical padding and a hairline between them, name at 15/500 over a 13 px hint on the left, the control on the right. One 36 px control family carries every row's control at 12 px radius with a `--hairline2` stroke — `Select`, `NumberField` (`task-36` moved it, with `Segmented`, out of `SettingsRow.tsx` into `client/src/components/ui/`), a text field and a button — and the on/off rows plus the theme and density pickers take `Switch`'s pill shape instead: a recessed `--steel` track under a raised `--strip` option. Density, text size and the staleness window already draw through `Segmented` (`SettingsView.tsx:192-210`) and gain nothing here but this skin, which `task-39` gave the primitive as a `pill` prop rather than as a restyle of its one class family: the Runs band's range control is the same component and §8.4.1 asks it for a stroked chip, so the two surfaces disagree through a prop (the design spec's §12.1) instead of through a second family. The theme picker's own swatch buttons (`.set-theme`, `.set-swatch`, `SettingsView.tsx:169-188`) sit on that same track-and-option ground now rather than on the bordered tile they had. The swatches themselves keep their job at 24 px and 8 px radius — this design's own figure, not the 34 px strip at a 2 px corner they were (`client/src/styles.css:695`).

Two hand-balanced columns place the cards, by hand rather than by a masonry reflow — a reflow would move a card to the other side of the page as its neighbour above grew a row, so a reader who learnt where `Claude Agents` sits would find it somewhere else after a knob changed. The split is the orchestrator pair on the right (`Orchestrator · this device` over `Orchestrator watchdog · this server`) and the other three on the left, which balances the two sides and keeps the one adjacency that carries meaning: those two are the same subject read at two scopes — what a run is started with, then what happens to one that has already crashed — and a reader meets them in that order. It folds to one column under 1100 px, above the 700 px phone break every other split on this board uses; the 700 px block Settings does have is about the ROW, stacking a control under its label, and says nothing about the grid.

Settings is TWO pages since task-42 — Local and Shared — picked from the rail's sub-nav tree (§8.0) and from nothing else. The band's right slot, empty until now because every control on this page belongs to a row inside a card, carries one thing that is not a control: a scope pill stating which backend the page's cards write to. Both pages use the existing `Pill` primitive at `tone="neutral"` and let the word carry the distinction — `this browser` on Local, `this machine` on Shared — rather than the green-tinted Shared pill the dashboard this pattern came from draws: `PillTone`'s five members each name a *reading* (`live`, `warn`, `bad`, `done`, `neutral`), none of them means "shared", and minting a second pill look inside the settings block would be a second home for a primitive, which §12.1 forbids and `test/design-guards.test.ts`'s guard 7 pins. The band's subtitle changes with the page, because the one-line summary is now the page's own. Both pages draw the same two hand-balanced columns described above; only the balance differs — Local is two short cards (`Display`, `Board`) against two taller ones (`Orchestrator`, `Dispatch`), Shared is the watchdog's long card against two shorter host reports stacked in one column (`Claude Agents · this machine` over `Trackers · this machine`, added by task-45 — both report and neither sets). There is NO in-page switch at any width: the rail's trees stand open below 700 px, so a second control here would be a second wording free to disagree with the tree's, which is the same rule Runs' two pages have followed since task-38. The `Claude Agents` card splits to make the arrangement honest — its three per-device rows become `Dispatch · this device` on Local, carrying `open dashboard ↗` onto the `Dashboard link` row that supplies its href, and the status row that stays behind on Shared has no control in its right slot at all.

The watchdog group keeps its four rows exactly as `WatchdogGroup.tsx` already writes them — Enabled, Check every, Leave a resumed run alone for, Give up after. What changes is the orientation row above them: the `Live view` line was prose with no control; `task-39` made it a real link, opening Runs › Watchdog through the same pair the rail's sub-nav tree calls (§8.0) — `setRunsMode` and then the shell's own section setter, both handed down from `App` as one prop so the link and the rail cannot come to disagree about the destination.
...
Facts: project dot plus name, id, section, created / updated / last commit, `groomed`, tags, `in progress since <started>` with its elapsed reading while a session holds the item, elapsed and token counters when present, the file path, the dispatch control — and under it, for a live tracker claim that is not a run's, **Stop & release** or **Release claim** (#225, `ClaimRelease`), a 28 px chip whose confirmation is `ui/Confirm` drawn inline in its place, never a second overlay. Body: the rendered Markdown at §8.1's scale — 15/400 body, 19/500 headings, code spans on `--steel`.

There is no second modal, and the reason is where the run's own reading already lives. Everything a run modal would have drawn is §8.4.1's detail sheet, always beside the list rather than behind a click — which is also why that sheet, and not a modal, is the one surface able to carry `RunControls` at all: a modal a reader opens and closes has no standing place to hold a Pause or a Resume across the length of a run the way an always-visible sheet does. Its contents stay §8.4.1's alone; this section names it only to say why `Modal` is composed by the one surface that needs opening and closing, the item, and not by two. The Stop's confirmation (bug-53) is not a second modal either: `Confirm` draws inline, in place of the chip that opened it, with no scrim and no lift — a box over the Runs page would sit on top of the very run it is asking about.
...
`useDialogEscape` (`client/src/hooks/useDialogEscape.ts`) is untouched in mechanism: one owner, a module-level LIFO stack, the topmost entry closes and nothing else. What changes is the count it ranks. Its own comment and the `invariants.md` entry it backs both named four dialogs before this redesign — `ItemDrawer`, `LaunchSheet`, `RunDrawer`, `OrchestrateSheet` — and both now read **three**: the item modal and these two sheets. One further step landed with the overlays: the three no longer call the hook themselves, the two SHELLS do (`Modal`, `FormSheet`), so a fourth surface opened through either joins the stack by construction rather than by remembering to. `RunDrawer` does not hand its slot to a renamed surface; it leaves the stack outright, because its content is now §8.4.1's detail sheet, sitting inline on the Runs page beside the list rather than opening and closing as a dialog at all — there is nothing left there for Escape to close. The `invariants.md` entry for this rule is edited in place to name three, the same treatment §8.4.1 gives the other entries a deleted surface used to carry, rather than deleted itself: the rule survives every surface that has ever carried it. `Confirm` joins the stack too while it is drawn, and is not counted among the three: it is not a dialog, but Escape is still the stack's key and not a listener of its own.
...
All three are silenced under `prefers-reduced-motion: reduce` through the same pattern this codebase already uses, not a mechanism written new for this redesign: a blanket `@media (prefers-reduced-motion: reduce)` rule (`client/src/styles.css:728`) cuts every animation's duration to near zero and runs it once, which freezes it on its LAST keyframe rather than cancelling it — wrong whenever that last frame is not the element's resting state. Three elements already needed the fix for exactly that reason: an explicit `animation: none` after the unconditional rule, in whichever reduced-motion block sits nearest it, carrying `!important` whenever that unconditional rule outranks it on specificity. `.board-card-stage-glyph` is the worked example (`:735`): its unconditional rule is `.board-card-stage .board-card-stage-glyph` (`:464`), two classes deep, so plain source order cannot win and the override needs the flag. `.watchdog-lamp-armed` needs it too, in its own block (`:1443`). `.run-track-dot-current` / `.run-track-node[data-in="live"]` need neither: each shares its selector with its own unconditional rule earlier in the same section, so source order alone settles it (`:2180-2181`). The three this section names carry the same explicit override, each judged per selector: `.ui-dot-breathe` (`:2373`), the stage track's `.run-track-dot-current` (`:2181`) and `.run-controls-needs-you` (`:877`), none of them needing the flag, because each shares its single-class selector with the unconditional rule it corrects and comes after it. `test/motion-style.test.ts` drives all three from one table, and asserts the glyph's `!important` beside them so that "only where it is needed" stays a judgement rather than a habit. (Every line number here is a pointer, not a contract — they move.)
