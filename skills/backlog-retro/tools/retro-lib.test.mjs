...
import { claudeProjectKey, claudeProjectsRoot, decodeProjectDir, orchHome, projectDir, readRegistryNames, registryFile, retroHome } from './lib/paths.mjs';
...
test('claudeProjectKey mirrors Claude Code’s own project directory key', () => {
  ...
  // not a `split('/').join('-')`.
  assert.equal(claudeProjectKey('/Users/a/Documents/custom-projects/backlog-manager'), '-Users-a-Documents-custom-projects-backlog-manager');
...
test('stageSpans differences consecutive stamps and keeps queue wait out of the pipeline', async () => {
  ...
  assert.deepEqual(Object.keys(stages).sort(), ['dispatched', 'fixing', 'inspecting', 'merging', 'preflight', 'reviewing', 'verifying']);
...
function writeLog(file, { contexts = [], result, sessionId = 'sess-x' } = {}) {
  const lines = [JSON.stringify({ type: 'system', subtype: 'init', session_id: sessionId, cwd: '/tmp' })];
...
function syntheticSessions(n) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const costUsd = t.cacheRead * TEST_RATES.cacheRead + t.cacheCreation * TEST_RATES.cacheCreation + t.output * TEST_RATES.output + t.input * TEST_RATES.input;
...
test('priceTokens multiplies each token count by its own rate', async () => {
  ...
  assert.equal(priceTokens({ cacheRead: 2e6, cacheCreation: 0, output: 0, input: 0 }, TEST_RATES), 1.0);
});
...
test('readDriver reports each way a transcript is missing with its own reason', async (t) => {
  ...
  assert.deepEqual(readDriver({ runId: 'R0', project, driverSessionId: null }, root, TEST_RATES), { runId: 'R0', project, driver: null, reason: 'no-lease' });
...
test('computePrevious leaves a delta null when either side is unmeasured', async (t) => {
  ...
  writeRecord(
    dir,
    '2026-09-06T21:03:30.697Z',
    { ...RECORD_TOTALS, itemWallMin: { median: null } },
    {
      reviews: {},
      candidates: []
    }
