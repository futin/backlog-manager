...
export function fitRates(sessions) {
  ...
  rates.maxResidualUsd = Math.max(...rows.map(({ x, y }) => Math.abs(x.reduce((acc, v, i) => acc + v * solution[i], 0) - y)));
...
export function priceTokens(tokens, rates) {
  ...
  return RATE_KEYS.reduce((acc, key) => acc + (Number.isFinite(tokens?.[key]) ? tokens[key] : 0) * rates[key], 0);
}
