...
function headline(sweep) {
  ...
  lines.push(row('driver spend', t.costUsd.driverEstimated === null ? 'not priced — no rate fit' : `${money(t.costUsd.driverEstimated)} est.`));
  lines.push(row('all-in spend', t.costUsd.allIn === null ? 'not priced — no rate fit' : `${money(t.costUsd.allIn)} est.`));
  lines.push(
    row(
      'per merged item',
      t.costPerMerged.measured === null
        ? '—'
        : `${money(t.costPerMerged.measured)} measured` + (t.costPerMerged.allIn === null ? '' : `, ${money(t.costPerMerged.allIn)} est. all-in`)
    )
  );
  lines.push(
    row(
...
function previousBlock(sweep) {
  ...
  if (sweep.previous.labelRates) {
    lines.push(
      row(
        'labels then',
        Object.entries(sweep.previous.labelRates)
          .map(([k, v]) => `${k} ${pct(v, 1)}`)
          .join(', ')
      )
    );
