...
const config: Config = {
  ...
  // Timezone pin. Must be `globalSetup` rather than a `setupFiles` entry: the
  // latter runs inside the test context, where `process.env` is jest's own
  // copy and an assignment to `TZ` never reaches Node's timezone cache. See
