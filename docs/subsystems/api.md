...
| `release`   | `project, id, commentId, session, reason, counters?, runId?, host?` | edits `released` into the claim, writes `counters` verbatim, removes `in-progress`; `runId` (bug-42) is the RUN asserting it owns the claim, which is what lets an abort session release a live claim it does not hold | 200 `{ commentId, record }`                    |
