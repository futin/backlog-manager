...
- Read: spec §3; mock `docs/superpowers/specs/2026-09-15-fe-redesign-mockups/01-board-shape.html` (option A) and `02-runs-on-board.html` (option C)
...
**Files:**
- Modify: `.claude/DESIGN.md` (append)
- Read: spec §4, §8 (rules that move); mocks `03-runs-shape.html` (option C), `04-watchdog.html` (option A)
...
Required content of 8.4.1, top to bottom: band (`Runs`, `2 live · 29 past`, project select and range control as chips); **one live card per** `running` / `paused` / `starting` — `--strip` sheet, 16 px radius, 24 px padding, two columns with `RunControls` (Pause / Cancel / Resume, plus Open) as 28 px chips right, Cancel in `--red` text; left column contents (dot + project 19/500 + run id and start time 12 `--ink3` + mode pill; §7 progress row 10 px `--fill-progress` hatched over `--steel`, two-tone `2 / 5`, elapsed and heartbeat at 12; hairline; current item head — id 12 `--ink3`, title 14/500, stage pill right — and its stage track); the `paused` variant (`paused after <id>`), the `starting` variant (`--fill-live` dot, `starting…`, age, no controls), the **crashed** variant (`--red` dot, `crashed` pill, the `run-watchdog.ts` clause as a 13 px sentence, Resume exactly when `watchdogStoodDown` allows, and that `watchdog-coupling.test.tsx` drives this card from now on); **figure strip** — one sheet the ground divides, five figures on a 2 px `--board` gap grid wrapping 5→3→2 with the seam drawn both ways, label 13 `--ink2`, value 30/700, 12 px line, the five names (runs; completed / queued; avg item work, queue wait excluded; rework / completed; verify pass), the sixth full-width cell holding `StageBars` at 10 px bar height; **history sheet** — title + subtitle, 13/500 `--ink2` day kickers, 44 px rows (dot tones `--ink3` done / `--red` crashed / `--fill-live` paused; project 14/500; items touched 13 `--ink2`; two-tone count; wall time 12 `--ink3`; mode pill; `›`), load-more as a flat chip, a row opens the run modal (§8.7), live runs not in the list. Then the **rules that moved here**, as a table copied in substance from spec §8: crashed renders as crashed; Resume for crashed lives here alone and paused Resume collapses here; the resume coupling; `starting` visible before its run file exists; the Orchestrate control's hide on starting unchanged; polling unchanged.

- [ ] **Step 2: Write 8.4.2**

Required content: band `Runs · Watchdog` with the state line as subtitle and a flat `Policy in Settings ›` chip (policy edited there only); **three-figure strip** — Sweeper (phase as a 24/700 word, 6 px sweep meter in `--fill-progress` hatched, `next sweep in … · every 10m`), Watching (count, `1 crashed · 1 heartbeating`), Policy (`check every / leave alone for / give up after` as 12-over-14/500 pairs, the enabled switch's state as the 12 px line); **Watching sheet** — one row per `running` run in the runs payload annotated from `watching`, skew rendered not hidden (`· not yet watched`; a placeholder row for a watched id with no run), with dot, project 14/500, run id 12 `--ink3`, heartbeat reading, `n / N attempts`, verdict as glyph + words in the kind's tone, `Resume now` chip when `watchdogStoodDown` allows; **Activity sheet** — `time · kind · run · detail`, th 12/400 `--ink2`, td 13, time 12 `--ink3`, kind cell = `WATCHDOG_KIND_GLYPH` + word coloured by `WATCHDOG_KIND_TONE` (`live` `--green`, `done` `--green`, `bad` `--red`, `warn` `--amber`, `muted` `--ink3`), scrolls inside its own sheet under 700 px. State why it stays a page (the user's pick over folding into cards): the console is read during a run, and its Watching set is the sweeper's, one tick of skew aside.
...
const must=["useWatchdog","RunControls","--fill-progress","--steel","30/700","2 px","1100","700","44 px","StageBars","watchdogStoodDown","watchdog-coupling.test.tsx","Runs · Watchdog","Policy in Settings","WATCHDOG_KIND_GLYPH","WATCHDOG_KIND_TONE","not yet watched","StageTrack","×N","--hairline2","03-runs-shape.html","04-watchdog.html"];
const miss=must.filter(s=>!t.includes(s)); console.log(miss.length?"MISSING: "+miss.join(", "):"ALL PRESENT")'
```
Expected: `1`, `3`, `ALL PRESENT`.
...
**Files:**
- Modify: `.claude/DESIGN.md` (append)
- Read: spec §5, §6; `../claude-agents-dashboard/.claude/DESIGN.md` §8.2, §8.6, §8.7 (the passages being restated)
...
const must=["capture","--ink3","Board · this device","Claude Agents · this machine","36 px","--steel","1100","Live view","290 px","1080","calc(100vw / var(--font-scale) - 48px)","620","useDialogEscape","prefers-reduced-motion","Branches to merge","uncommitted"];
