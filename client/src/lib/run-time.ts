 ...
 * there was reporting nothing in progress at all, and `null` is what lets
 * each caller say so in its own words (`RunStrip`'s crashed strip prints
 * "all items at rest", `WatchdogMonitor`'s row prints "between items")
 * instead of naming whichever untouched entry happens to sit first in that
 * tail.
 *
 * task-18 lifted this out of `RunStrip.tsx` the moment the monitor became
 * its second caller: the rule above is a genuine judgement about queue
 * order, not a one-liner, and two surfaces answering "what is this run
 * working on" from two copies of it is the drift this module already
 * exists to prevent for every other run reading.
 */
export function lastReportedEntry(queue: readonly RunQueueItem[]): RunQueueItem | null {
