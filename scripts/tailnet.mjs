...
export const serveFailureRemedies = () => ['tailscale status', 'sudo tailscale set --operator=$USER', 'tailscale up'];
...
async function up(port, cli, dryRun) {
  ...
  if (!(await listening(port))) {
    ...
    process.stdout.write(`warning: nothing is listening on 127.0.0.1:${port} — start the stack with \`pnpm run docker:up\`\n`);
  }
  ...
  process.stdout.write('reachable from anywhere you are logged into the tailnet, while this machine is awake\n');
  return 0;
}
...
export async function main(argv) {
  ...
  const cli = findCli();
  if (!cli && !dryRun) {
    process.stderr.write(`tailscale CLI not found. Looked in:\n${CLI_CANDIDATES.map((p) => `  ${p}`).join('\n')}\n`);
    return 2;
  }
