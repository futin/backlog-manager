...
function main() {
  ...
  const blocker = publishBlocker({
    dirty: git(['status', '--porcelain', '--', ...PUBLISHED_PATHS])
      .split('\n')
      .filter(Boolean),
  ...
  if (stillDrifted.length > 0) {
    ...
    if (absent.length > 0) {
      ...
      console.error(
        'Not known_marketplaces.json — that file is a cache reconciled from the above on the next session start, so an edit to it is reverted (and is itself what triggers the revert).'
      );
